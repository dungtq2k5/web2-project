...
class Auths  {
  ...
  public function verifyAction(string $action_code): void {
    // DEV only, temp comment for dev
    // $decoded = $this->jwt["token"] ? $this->verifyJWT() : false;
    // if(!$decoded) {
    ...
    //   WHERE usr.id = :id AND per.action_code = LOWER(:action_code)
    // )";
  ...
  public function genJWT(string $usr_id): bool {
    if($this->jwt["token"] && $this->verifyJWT()) return false; //Handle re-login
  ...
  public function verifyJWT(): stdClass | false {
    try {
      $decoded = JWT::decode($this->jwt["token"], new Key($this->jwt["secret_key"], $this->jwt["hash_algor"]));
      return $decoded;
    } catch(Exception $e) {

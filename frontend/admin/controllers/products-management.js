...
async function renderProductsData(productsList = null) {
  tbody.html(
    "<tr><td colspan='11' class='p-3 text-center'>Loading data...</td></tr>"
  );
  ...
  try {
    ...
    for (const [idx, product] of products.entries()) {
      ...
      dataHTML += `
        ...
        >
          ...
          <td data-cell="description" class="p-2 small">${
            product.description
          }</td>
          <td data-cell="stop selling" class="text-center p-2">
            ${
              product.stop_selling == 1
    ...
    tbody.html(
      dataHTML ||
        "<tr><td colspan='11' class='p-3 text-center'>No data found!</td></tr>"
    );
    ...
    tbody.html(
      `<tr><td colspan='11' class="p-3 text-center table-danger">Error loading data: ${error.message}</td></tr>`
    );
  }
}
...
async function renderProductVariationsPopup(number, id) {
  disableBgScroll();
  backdrop.html(
    "<div class='d-flex justify-content-center align-items-center' style='height: 100vh;'><div class='spinner-border text-primary' role='status'><span class='visually-hidden'>Loading product variations...</span></div></div>"
  );
  backdrop.show();
  ...
  try {
      ...
      .map((variation) => {
        ...
        <li class="list-group-item p-3">
          <div class="row align-items-center">
            ...
            <div class="col-md-10">
              <div class="row small">
                <div class="col-sm-6 col-md-4"><strong>ID:</strong> ${
                  variation.id
                }</div>
                <div class="col-sm-6 col-md-4"><strong>Watch Size:</strong> ${
                  variation.watch_size_mm
                }mm</div>
                <div class="col-sm-6 col-md-4"><strong>Watch Color:</strong> <input type="color" value="${
                  variation.watch_color
                }" disabled class="form-control form-control-sm" style="width: 25px; height: 25px; padding: .1rem"></div>
                <div class="col-sm-6 col-md-4"><strong>Stock:</strong> ${
                  variation.stock_quantity
                }</div>
                <div class="col-sm-6 col-md-4"><strong>Sell Price:</strong> ${
                  variation.price_cents
                }&#162;</div>
                <div class="col-sm-6 col-md-4"><strong>Base Price:</strong> ${
                  variation.base_price_cents
                }&#162;</div>
                <div class="col-sm-6 col-md-4"><strong>Display:</strong> ${
                  variation.display_size_mm
                }mm, ${variation.display_type}</div>
                <div class="col-sm-6 col-md-4"><strong>Resolution:</strong> ${
                  variation.resolution_h_px
                }x${variation.resolution_w_px}px</div>
                <div class="col-sm-6 col-md-4"><strong>Memory:</strong> ${
                  variation.ram_bytes
                }B RAM / ${variation.rom_bytes}B ROM</div>
                <div class="col-sm-6 col-md-4"><strong>OS:</strong> ${
                  variation.os.name
                }</div>
                <div class="col-sm-6 col-md-4"><strong>Connectivity:</strong> ${
                  variation.connectivity
                }</div>
                <div class="col-sm-6 col-md-4"><strong>Battery:</strong> ${
                  variation.battery_life_mah
                }mAh</div>
                <div class="col-sm-6 col-md-4"><strong>Water Resist:</strong> ${
                  variation.water_resistance_value
                } ${variation.water_resistance_unit}</div>
                <div class="col-sm-6 col-md-4"><strong>Sensor:</strong> ${
                  variation.sensor
                }</div>
                <div class="col-sm-6 col-md-4"><strong>Case:</strong> ${
                  variation.case_material
                }</div>
                <div class="col-sm-6 col-md-4"><strong>Band:</strong> ${
                  variation.band_material
                }, ${variation.band_size_mm}mm, <input type="color" value="${
          variation.band_color
        }" disabled class="form-control form-control-sm d-inline-block" style="width: 25px; height: 25px; padding: .1rem"></div>
                <div class="col-sm-6 col-md-4"><strong>Weight:</strong> ${
                  variation.weight_milligrams
                }mg</div>
                <div class="col-sm-6 col-md-4"><strong>Released:</strong> ${convertUtcToLocalDatetime(
    ...
    backdrop.html(`
      <div class="card shadow-lg m-3 m-md-5 mx-auto" style="max-width: 900px; max-height: 90vh;">
        ...
        <div class="card-body overflow-auto">
          ${
            variationsHTML
              ? `<ul class="list-group list-group-flush">${variationsHTML}</ul>`
    ...
    backdrop
      .find(".js-variations-popup-close-btn")
      .click(() => closePopup(backdrop));
  } catch (error) {
    console.error(`Error: ${error.message}`);
    backdrop.html(
      `<div class="alert alert-danger m-3">Error loading product variations: ${error.message}</div>`
    );
  }
}
...
async function renderUpdateProductForm(number, id) {
  disableBgScroll();
  backdrop.html(
    "<div class='d-flex justify-content-center align-items-center' style='height: 100vh;'><div class='spinner-border text-primary' role='status'><span class='visually-hidden'>Loading update product form...</span></div></div>"
  );
  backdrop.show();
  ...
  try {
    ...
    const img =
      product.image_name && `${PRODUCT_IMG_PATH}/${product.image_name}`;
          ...
          <div class="card-body">
            ...
            <div class="form-check mb-3">
              <input
                ...
                ${product.stop_selling == 1 ? "checked" : ""}
              >
    ...
    backdrop
      .find(".js-update-product-form-close-btn")
    ...
    form.submit(async (e) => {
      ...
      if (await validateForm()) {
        ...
        const res = await updateProduct(
          id,
          formData.get("image") && formData.get("image")["name"] !== "null"
            ? formData
            : Object.fromEntries(formData) // Only use FormData within file
        );

        if (res.success) {
          crudProductMsg.text(
            `* product id ${id} - number ${number} was updated`
          );
          setTimeout(() => {
            crudProductMsg.text("");
          }, DISPLAY_MSG_TIMEOUT);
    ...
    console.error(`Error: ${error.message}`);
    backdrop.html(
      `<div class="alert alert-danger m-3">Error loading product data: ${error.message}</div>`
    );
  }
}
...
function renderDeleteProductPopup(number, id) {
  ...
  backdrop.find(".js-delete-product-popup-close-btn").click(() => {
    closePopup(backdrop);
  });
  ...
  submitBtn.click(async () => {
    ...
    if (res.success) {
      crudProductMsg.text(`* product id ${id} - number ${number} was deleted`);
      setTimeout(() => {
        crudProductMsg.text("");
      }, DISPLAY_MSG_TIMEOUT);
      renderProductsData(); //Re-render
...
async function renderCreateProductForm() {
  disableBgScroll();
  backdrop.html(
    "<div class='d-flex justify-content-center align-items-center' style='height: 100vh;'><div class='spinner-border text-primary' role='status'><span class='visually-hidden'>Loading create product form...</span></div></div>"
  );
  backdrop.show();
  ...
  try {
    ...
    backdrop
      .find(".js-create-product-form-close-btn")
      .click(() => closeForm(backdrop));
    ...
    console.error(`Error: ${error.message}`);
    backdrop.html(
      `<div class="alert alert-danger m-3">Error loading create product form: ${error.message}</div>`
    );
  }
}
...
async function renderCreateVariationForm(productNumber, productId) {
  disableBgScroll();
  backdrop.html(
    "<div class='d-flex justify-content-center align-items-center' style='height: 100vh;'><div class='spinner-border text-primary' role='status'><span class='visually-hidden'>Loading create variation form...</span></div></div>"
  );
  backdrop.show();
  ...
  try {
    ...
    const osHTML = osList
      .map((os) => `<option value="${os.id}">${os.name}</option>`)
      .join("");
    ...
    backdrop
      .find(".js-create-variation-form-close-btn")
      .click(() => closeForm(backdrop));
    ...
    form.submit(async (e) => {
      ...
      if (await validateForm()) {
        ...
        if (res.success) {
          crudProductMsg.text(
            `* new variation of product id ${productId} - number ${productNumber} was created`
          );
          setTimeout(() => {
            crudProductMsg.text("");
          }, DISPLAY_MSG_TIMEOUT);
    ...
    console.error(`Error: ${error.message}`);
    backdrop.html(
      `<div class="alert alert-danger m-3">Error loading create variation form: ${error.message}</div>`
    );
  }
}

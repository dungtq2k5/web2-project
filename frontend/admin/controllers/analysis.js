...
function renderSearchAnalysisForm() {
  ...
  form.submit(async (e) => {
    ...
    const filteredUsersReceivedOrders = await getUsersReceivedOrdersList(
      ...
      formData.get("top_customers")
        ? parseInt(formData.get("top_customers"))
        : null
    );
  ...
  try {
    ...
    tbody.html(() => {
      ...
      usersOrders.forEach((userOrders, idx) => {
        const receivedOrdersHTML = userOrders.received_orders
          .map((order) => {
            const delAddress = formatAddress(order.delivery_address);

            return `
            <li class="mb-3 p-3 border rounded bg-light shadow-sm">
              <div class="d-flex justify-content-between"><span><strong>Order ID:</strong></span><span>${
                order.id
              }</span></div>
              <div class="d-flex justify-content-between"><span><strong>Total:</strong></span><span>${
                order.total_cents
              }&#162;</span></div>
              ...
              <div class="d-flex justify-content-between"><span><strong>Order at:</strong></span><span>${convertUtcToLocalDatetime(
                order.order_date
              )}</span></div>
              <div class="d-flex justify-content-between"><span><strong>Est. received:</strong></span><span>${convertUtcToLocalDatetime(
                order.estimate_received_date
              )}</span></div>
              <div class="d-flex justify-content-between"><span><strong>Received:</strong></span><span>${convertUtcToLocalDatetime(
                order.received_date
              )}</span></div>
              <div class="d-flex justify-content-between align-items-center mt-2">
                <div><span><strong>Total items:</strong> ${
                  order.items.length
                }</span></div>
                <button
        ...
        dataHTML += `
          <tr class="align-middle">
            ...
            <td data-cell="total spent" class="text-end p-2">${
              userOrders.total_cents
            }&#162;</td>
          </tr>
        `;
      });
...
async function renderOrderItemsDetailPopup(orderId) {
  disableBgScroll();
  backdrop.html(
    "<div class='d-flex justify-content-center align-items-center' style='height: 100vh;'><div class='spinner-border text-primary' role='status'><span class='visually-hidden'>Loading order items data...</span></div></div>"
  );
  backdrop.show();
  ...
  try {
    ...
    for (const item of items) {
      ...
      const itemDetailsHTML = item.product_instances
        .map(
          (instance) =>
    ...
    backdrop
      .find(".js-order-items-detail-close-btn")
      .click(() => closePopup(backdrop));
  } catch (error) {
    console.error(`Error: ${error.message}`);
    backdrop.html(
      `<div class='alert alert-danger m-3'>Error loading order items data: ${error.message}</div>`
    );
  }
}
...
async function getUsersReceivedOrdersList(
  ...
  const groupOrdersByUsersId = async () => {
    ...
    for (const order of ordersList) {
      if (orderFrom || orderTo) {
        ...
        if (
          (orderFrom && orderDate < orderFrom) ||
          (orderTo && orderDate > orderTo)
        )
          continue;
      }
  ...
  if (getTop) {
    // Get top -> sort by desc -> slice -> order by

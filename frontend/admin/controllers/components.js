...
export async function renderUserDetailPopup(userId, backdropEle) {
  disableBgScroll();
  backdropEle.html(
    "<div class='d-flex justify-content-center align-items-center' style='height: 100vh;'><div class='spinner-border text-primary' role='status'><span class='visually-hidden'>Loading data...</span></div></div>"
  );
  backdropEle.show();
  ...
  try {
    ...
    const rolesHTML = user.roles
      .map((role) => `<li class="list-group-item">${role.name}</li>`)
      .join("");
            ...
            <div class="row">
              <div class="col-md-6">
                ...
                <ul class="list-group list-group-flush">
                  <li class="list-group-item d-flex justify-content-between"><span><strong>ID:</strong></span> <span>${
                    user.id
                  }</span></li>
                  <li class="list-group-item d-flex justify-content-between"><span><strong>Full Name:</strong></span> <span>${
                    user.full_name
                  }</span></li>
                  <li class="list-group-item d-flex justify-content-between"><span><strong>Email:</strong></span> <span>${
                    user.email
                  }</span></li>
                  <li class="list-group-item d-flex justify-content-between"><span><strong>Phone:</strong></span> <span>${
                    user.phone_number || "N/A"
                  }</span></li>
                  <li class="list-group-item d-flex justify-content-between"><span><strong>Created At:</strong></span> <span>${convertUtcToLocalDatetime(
                    user.created_at
                  )}</span></li>
                  <li class="list-group-item d-flex justify-content-between"><span><strong>Updated At:</strong></span> <span>${convertUtcToLocalDatetime(
                    user.updated_at
                  )}</span></li>
                </ul>
              </div>
    ...
    backdropEle
      .find(".js-user-detail-popup-close-btn")
      .click(() => closePopup(backdropEle));
  } catch (error) {
    console.error(`Error: ${error.message}`);
    backdropEle.html(
      `<div class="alert alert-danger m-3">Error loading user data: ${error.message}</div>`
    );
  }
}
 ...
 * @returns none.
 */
export async function renderProductVariationDetailPopup(
  variationId,
  backdropEle,
  orderId = null
) {
  // If has orderId -> display product instances
  disableBgScroll();
  backdropEle.html(
    "<div class='d-flex justify-content-center align-items-center' style='height: 100vh;'><div class='spinner-border text-primary' role='status'><span class='visually-hidden'>Loading data...</span></div></div>"
  );
  ...
  try {
    ...
    const productHTML = `
      <div class="col-lg-4 border-end p-3">
        ...
        <ul class="list-unstyled small">
          ...
          <li><strong>Description:</strong> <p class="small">${
    ...
    const variationHTML = `
      <div class="col-lg-5 border-end p-3">
        ...
        <ul class="list-unstyled small row">
          ...
          <li class="col-sm-6"><strong>Size:</strong> ${
            variation.watch_size_mm
          }mm</li>
          <li class="col-sm-6"><strong>Color:</strong> <input type="color" value="${
            variation.watch_color
          }" disabled class="form-control form-control-sm" style="width: 50px; height: 20px;"></li>
          <li class="col-sm-6"><strong>Stock:</strong> ${
            variation.stock_quantity
          }</li>
          <li class="col-sm-6"><strong>Price:</strong> ${
            variation.price_cents
          }&#162;</li>
          <li class="col-sm-6"><strong>Base Price:</strong> ${
            variation.base_price_cents
          }&#162;</li>
          <li class="col-sm-6"><strong>Display:</strong> ${
            variation.display_size_mm
          }mm, ${variation.display_type}</li>
          <li class="col-sm-6"><strong>Resolution:</strong> ${
            variation.resolution_h_px
          }x${variation.resolution_w_px}px</li>
          <li class="col-sm-6"><strong>Memory:</strong> ${
            variation.ram_bytes
          }B RAM / ${variation.rom_bytes}B ROM</li>
          <li class="col-sm-6"><strong>OS:</strong> ${variation.os.name}</li>
          <li class="col-sm-6"><strong>Connectivity:</strong> ${
            variation.connectivity
          }</li>
          <li class="col-sm-6"><strong>Battery:</strong> ${
            variation.battery_life_mah
          }mAh</li>
          <li class="col-sm-6"><strong>Water Resist:</strong> ${
          ...
          <li class="col-sm-6"><strong>Case:</strong> ${
            variation.case_material
          }</li>
          <li class="col-sm-6"><strong>Band:</strong> ${
            variation.band_material
          }, ${variation.band_size_mm}mm <input type="color" value="${
      variation.band_color
    }" disabled class="form-control form-control-sm" style="width: 50px; height: 20px;"></li>
          <li class="col-sm-6"><strong>Weight:</strong> ${
            variation.weight_milligrams
          }mg</li>
          <li class="col-sm-6"><strong>Released:</strong> ${convertUtcToLocalDatetime(
    ...
    const instancesHTML = orderId
      ? await genInstancesHTML(orderId, variationId, true)
      : ""; // Pass true for styled output
        ...
        <div class="card shadow-lg m-3 m-md-5 mx-auto" style="max-width: 1200px;">
          ...
          <div class="card-body">
            <div class="row">
              ...
              ${
                instancesHTML
                  ? `<div class="col-lg-3 p-3">${instancesHTML}</div>`
                  : ""
              }
            </div>
          </div>
    ...
    backdropEle
      .find(".js-product-detail-popup-close-btn")
      .click(() => closePopup(backdropEle));
  } catch (error) {
    console.error(`Error: ${error.message}`);
    backdropEle.html(
...
async function genInstancesHTML(orderId, variationId, styled = false) {
  // Support for renderProductVariationDetailPopup()
  const orderItem = await getOrderItem(orderId, variationId);
  if (!styled) {
    const instancesListHTML = orderItem.product_instances
      .map(
        (instance) =>
          `<li>
        <p>${instance.sku}</p>
  ...
  const instancesListHTML = orderItem.product_instances
    .map(
      ...
      <div class="d-flex justify-content-between"><span><strong>SKU:</strong></span> <span>${
        instance.sku
      }</span></div>
      <div class="d-flex justify-content-between"><span><strong>Serial:</strong></span> <span>${
        instance.supplier_serial_number
      }</span></div>
      <div class="d-flex justify-content-between"><span><strong>IMEI:</strong></span> <span>${
        instance.supplier_imei_number || "N/A"
      }</span></div>
    </li>`
    )
    .join("");
  ...
  return `
    <div>
      ...
      ${
        instancesListHTML
          ? `<ul class="list-group list-group-flush">${instancesListHTML}</ul>`
          : '<p class="text-muted small">No instances found for this item in the order.</p>'
      }
    </div>
  `;
}

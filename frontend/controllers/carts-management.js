    ...
    for(const [idx, user] of users.entries()) {
      ...
      dataHTML += `
        <tr class="align-middle">
          ...
          >
            ${user.id}
            <button class="js-view-detail-user-btn btn btn-sm btn-outline-primary ms-1" title="View user details" data-user-id="${user.id}">
              <i class="uil uil-user"></i> View user
            </button>
          </td>
          ...
          <td data-cell="actions" class="text-center p-2">
            ${
              ...
              ? `<button
                  ...
                  data-user-id="${user.id}"
                >
                  <i class="uil uil-edit"></i> Modify cart
                </button>`
            : "No actions"
            }
          </td>
  ...
  try {
    ...
    form.submit(async e => {
      ...
      if(await validateForm()) {
        ...
        if (await allUpdated()) {
          form.find("#mod-cart-form-msg").html("<p class=\"text-success\">Cart updated successfully!</p>");
          setTimeout(() => {
            closeForm(backdrop);
            renderCartsData();
          }, DISPLAY_MSG_TIMEOUT);
        } else {
          form.find("#mod-cart-form-msg").text("Failed to update cart. Please try again.");
        }
      }
    ...
    backdrop.html(
      `<div class="card shadow-lg m-3 m-md-5 mx-auto" style="max-width: 700px;">
        <div class="card-header d-flex justify-content-between align-items-center">
          <h2 class="h5 mb-0">Error</h2>
          <button type="button" class="btn-close js-mod-cart-form-close-btn" aria-label="Close"></button>
        </div>
        <div class="card-body">
          <p class="text-danger">Could not load cart details. ${error.message}</p>
        </div>
        <div class="card-footer text-end">
          <button type="button" class="btn btn-secondary js-mod-cart-form-close-btn">Close</button>
        </div>
      </div>`
    );
    backdrop.find(".js-mod-cart-form-close-btn").click(() => closeForm(backdrop));
  ...
  try {
    ...
    backdrop.find("#remove-item-confirm-btn").click(async () => {
      ...
      if (await deleteUserCart(userId, variationId)) {
        crudCartMsg.html(`<div class="alert alert-success">Item removed successfully!</div>`);
        setTimeout(() => {
          crudCartMsg.html("");
          closePopup(backdrop);
          renderCartsData();
    ...
    backdrop.html(
      `<div class="card shadow-lg m-3 m-md-5 mx-auto" style="max-width: 500px;">
        ...
        <div class="card-body">
          <p class="text-danger">Could not load item details for removal. ${error.message}</p>
        </div>
        <div class="card-footer text-end">
          <button type="button" class="btn btn-secondary js-remove-item-popup-close-btn">Close</button>
        </div>
      </div>`
    );
    backdrop.find(".js-remove-item-popup-close-btn").click(() => closePopup(backdrop));

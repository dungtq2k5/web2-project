...
import { getOrderItem } from "../controllers/orders.js";
    ...
    backdropEle.html(() => {
      return `
        <div class="user-detail-popup--g">
          <button class="form__close--g js-user-detail-popup-close-btn">
          ...
          <div class="user-detail-popup__info--g">
            <div class="user-detail-popup__detail--g">
              ...
              <div>
                <h4>created at</h4>
                <p>${convertUtcToLocalDatetime(user.created_at)}</p>
              </div>
              <div>
                <h4>updated at</h4>
                <p>${convertUtcToLocalDatetime(user.updated_at)}</p>
              </div>
            </div>
          ...
          <button class="js-user-detail-popup-close-btn">close</button>
        </div>
      `;
    });
  ...
  try {
    ...
    const productHTML = `
      <div class="product-detail-popup__detail--g">
        <h3>Product</h3>
        <img src="${productImg}" alt="smartwatch" loading="lazy">
        <div>
          <h4>id</h4>
          <p>${product.id}</p>
        </div>
    ...
    const variationHTML = `
       <div class="product-detail-popup__detail--g">
        <h3>Variation</h3>
        <img src="${variationImg}" alt="smartwatch variation" loading="lazy">
        <div>
          <h4>id</h4>
          <p>${variation.id}</p>
        </div>
        ...
        <div>
          <h4>watch color</h4>
          <input type="color" value="${variation.watch_color}" disabled></input>
        </div>
        <div>
        ...
        <div>
          <h4>band</h4>
          <p>${variation.band_material} | ${variation.band_size_mm}mm | <input type="color" value="${variation.band_color}" disabled></input></p>
        </div>
        <div>
          <h4>weight</h4>
        ...
        <div>
          <h4>release at</h4>
          <p>${convertUtcToLocalDatetime(variation.release_date)}</p>
        </div>
      </div>
    `;

    const instancesHTML = orderId ? await genInstancesHTML(orderId, variationId) : "";

    backdropEle.html(() => {
      return `
        <div class="product-detail-popup--g">
          <button class="form__close--g js-product-detail-popup-close-btn"><i class="uil uil-times"></i></button>
          ...
          <div class="product-detail-popup__info--g">
            ...
            ${instancesHTML ? dividerHTML+instancesHTML : ""}
          </div>

          <button class="js-product-detail-popup-close-btn">close</button>
        </div>
      `;
    });
...
async function genInstancesHTML(orderId, variationId) { // Support for renderProductVariationDetailPopup()
  ...
  const instancesListHTML = orderItem.product_instances.map(instance => (
    `<li>
      ...
      <i class="uil uil-times"></i>
      <p>${instance.supplier_imei_number || "none"}</p>
    </li>`
  )).join("");

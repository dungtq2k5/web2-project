...
export async function getVariationsByProductId(id, stopSelling = null) {
  ...
  return variationsList.filter((variation) => {
    ...
    if (stopSelling !== null && variation.stop_selling != stopSelling)
      return false;
...
export async function getFilterVariationsList(
  ...
  if (releaseFrom)
    releaseFrom = new Date(convertLocalToUtcDatetime(releaseFrom));
  ...
  const filteredVariationsList = variationsList.filter((variation) => {
    ...
    if (
      priceFrom &&
      Number(variation.price_cents) < Number(priceFrom) &&
      Number(variation.base_price_cents) < Number(priceFrom)
    )
      return false;
    if (
      priceTo &&
      Number(variation.price_cents) > Number(priceTo) &&
      Number(variation.base_price_cents) > Number(priceTo)
    )
      return false;

    if (releaseFrom && new Date(variation.release_date) < releaseFrom)
      return false;
...
export async function softUpdateVariationStock(id, amount = 1) {
  variationsList = variationsList.map((variation) =>
    variation.id == id

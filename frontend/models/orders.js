...
export async function getFilterOrdersList(
  ...
  let filterOrdersList = ordersList.filter((order) => {
    ...
    if (
      cityProvince &&
      order.delivery_address.city_province.toLowerCase() != cityProvince
    )
      return false;
    if (district && order.delivery_address.district.toLowerCase() != district)
      return false;
    if (ward && order.delivery_address.ward.toLowerCase() != ward) return false;
    if (street && order.delivery_address.street.toLowerCase() != street)
      return false;
  ...
  if (sortByAddress)
    filterOrdersList = await sortOrdersListByAddress(filterOrdersList);

  if (sortByDate)
    filterOrdersList = await sortOrdersListByDate(filterOrdersList);
...
export async function sortOrdersListByAddress(list = null) {
  // Sort by city > district > ward > street
  ...
  return orders.sort((orderA, orderB) => {
    ...
    const compareCity = delAddrA.city_province.localeCompare(
      delAddrB.city_province
    );
...
export async function sortOrdersListByDate(list = null) {
  // Sort by order date
  const orders = list || (await getOrdersList());
  ...
  return orders
    .map((order) => ({
      ...order,
      order_date_timestamp: new Date(order.order_date).getTime(),
    }))
    .sort(
      (orderA, orderB) =>
        orderB.order_date_timestamp - orderA.order_date_timestamp
    )
...
export async function getOrderItem(orderId, variationId) {
  ...
  return (
    order.items.find((item) => item.product_variation_id == variationId) ||
    undefined
  );
}

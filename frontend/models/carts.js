...
export async function getCartsList(limit=null, offset=null) { // Return a copy
...
export async function updateCart(cart) {
  ...
  const res = await updateData(`${CARTS_API_URL}?user_id=${cart.user_id}&product_variation_id=${cart.product_variation_id}`, null, cart);

  if(res.success) {
    if(!isFetch) {
      await fetchCarts();
    } else {
      const idx = cartsList.findIndex(c => c.user_id == cart.user_id && c.product_variation_id == cart.product_variation_id);
      if(idx !== -1) {
        cartsList[idx] = {...cartsList[idx], ...cart};
      } else {
        console.warn(`Couldn't find cart with user_id: ${cart.user_id} and product_variation_id: ${cart.product_variation_id}`);
      }
    }
  }
...
export async function deleteCart(userId, productVariationId=null) { // If productVariationId is null, delete all carts of this user
  ...
  if(res.success) {
    if(!isFetch) {
      ...
      const idx = cartsList.findIndex(c => c.user_id == userId && c.product_variation_id == productVariationId);
      if(idx !== -1) {
        cartsList.splice(idx, 1);
      } else {
        console.warn(`Couldn't find cart with user_id: ${userId} and product_variation_id: ${productVariationId}`);
      }
    }
  }
...
export async function getCart(userId, productVariationId) {
  ...
  return cartsList.find(c => c.user_id == userId && c.product_variation_id == productVariationId);
}
...
export async function getAvailableCartsList() {
  ...
  for(const cart of cartsList) {
    ...
    if(!variation || variation.stop_selling || variation.stock_quantity <= 0) {
      continue;
    }

...
async function renderCart() {
  itemsContainer.html(`<div class="list-group-item text-center p-5"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div><p>Loading cart items...</p></div>`);

  const carts = await getCartsList();

  if (carts.length === 0) {
    itemsContainer.html(`<div class="list-group-item text-center p-5"><p class="mb-0">Your cart is empty.</p><a href="./index.php?page=home" class="btn btn-sm btn-link">Start shopping!</a></div>`);
  ...
  for(const [idx, cart] of carts.entries()) {
    ...
    if (cart.quantity > maxStock && variation.stock_quantity > maxStock) { // If current quantity is > 10 but stock allows
        quantityOptionsHTML += `<option value="${cart.quantity}" selected>${cart.quantity}</option>`;
    } else if (cart.quantity > variation.stock_quantity) { // If current quantity is more than available stock
        quantityOptionsHTML += `<option value="${variation.stock_quantity}" selected>${variation.stock_quantity} (Max)</option>`;
    }

    dataHTML += `
      <div class="list-group-item ${isAvailable ? "" : "bg-light opacity-75"}" data-variation-id="${variation.id}">
        <div class="row align-items-center g-2">
          ...
          <div class="col-2 col-md-1 text-center">
            <img src="${variationImg}" style="max-width: 50px; max-height: 50px; object-fit: contain;" class="img-fluid rounded" alt="${product.name}">
          </div>
          <div class="col">
            <h6 class="mb-1 small fw-medium">${product.name} - ${variation.watch_size_mm}MM</h6>
            <p class="mb-1 text-muted" style="font-size: 0.8rem;">${product.model}</p>
            <p class="mb-0 fw-bold text-primary small">&#36;${centsToDollars(parseInt(variation.price_cents))} USD</p>
          </div>
          <div class="col-4 col-md-2">
            <label for="select-quantity-${idx}" class="form-label visually-hidden">Quantity</label>
            <select id="select-quantity-${idx}" class="form-select form-select-sm js-item-quantity" ${isAvailable ? "" : "disabled"}>${quantityOptionsHTML}</select>
          </div>
        </div>
        ${isAvailable
  ...
  itemsContainer.find(".js-remove-item-btn").click(async e => {
    ...
    const variationId = $(e.currentTarget).closest(".list-group-item").data("variation-id");
...
async function renderCheckoutForm() {
  ...
  checkoutFormContainer.html(`<div class="text-center p-3"><div class="spinner-border spinner-border-sm text-primary" role="status"><span class="visually-hidden">Loading...</span></div></div>`);
  ...
  try {
    ...
    if(carts.length === 0) {
      ...
      if (allCarts.length === 0) {
         checkoutFormContainer.html(`<p class="text-muted text-center p-3">Your cart is empty.</p>`);
      } else {
         checkoutFormContainer.html(`<p class="text-muted text-center p-3">No items available for checkout.</p>`);
      }
      return;
    }
    ...
    for(const cart of carts) {
      const variation = await getVariation(cart.product_variation_id);
      if (variation && !variation.stop_selling && (await getProduct(variation.product_id)) && !(await getProduct(variation.product_id)).stop_selling) {
    ...
    checkoutFormContainer.html(`
      ...
      <div class="d-grid">
        <a href="./index.php?page=checkout" class="btn btn-dark ${totalCents === 0 ? 'disabled' : ''}"><i class="uil uil-shield-check"></i> Proceed to Checkout</a>
      </div>
    `);

  } catch(error) {
    console.error(error);
    checkoutFormContainer.html(`<p class="text-danger text-center p-3">Could not load checkout summary.</p>`);
  }
}

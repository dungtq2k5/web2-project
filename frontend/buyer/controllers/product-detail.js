...
import { centsToDollars, bytesToMB, mgToGrams } from "../../utils.js";
...
async function renderProduct(variationId = null) {
  productsContainer.html(
    `<div class="col-12 text-center p-5"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div><p>Loading product...</p></div>`
  );
  productSpecsContainer.html(
    `<li class="list-group-item text-center"><div class="spinner-border spinner-border-sm text-primary" role="status"><span class="visually-hidden">Loading...</span></div> Loading specifications...</li>`
  );
  ...
  try {
    ...
    if (product?.stop_selling != 0) {
      productsContainer.html(
        `<div class="col-12"><p class="alert alert-warning text-center">Product not found or no longer available.</p></div>`
      );
      productSpecsContainer.html(
        `<li class="list-group-item text-center">Product not found.</li>`
      );
      return;
    }
    ...
    if (variations.length === 0) {
      productsContainer.html(
        `<div class="col-12"><p class="alert alert-info text-center">No variations available for this product.</p></div>`
      );
      productSpecsContainer.html(
        `<li class="list-group-item text-center">No variations available.</li>`
      );
      return;
    }
    ...
    const sizesHTML = variations
      .map(
        (variation) =>
          `<li class="list-inline-item">
          <button
            class="btn btn-sm ${
              variation.id == defaultVariation.id
                ? "btn-dark"
                : "btn-outline-dark"
            } js-variation-size"
    ...
    const productsHTML = `
      <div class="col-md-6 text-center">
        <img src="${variationImg}" alt="${product.name} - ${
      defaultVariation.watch_size_mm
    }MM" class="img-fluid rounded shadow-sm mb-3" style="max-height: 400px; object-fit: contain;">
      </div>
      <div class="col-md-6">
        <div class="card shadow-sm">
          <div class="card-body">
            ...
            <p class="fs-4 fw-bold text-primary mb-3">&#36;${centsToDollars(
              parseInt(defaultVariation.price_cents)
            )} USD</p>
            ...
            <div class="d-grid gap-2 d-md-flex btn-group-sm" data-variation-id="${
              defaultVariation.id
            }">
    ...
    const specsArray = [
      ...
      {
        label: "Watch color",
        value: `<input type="color" value="${defaultVariation.watch_color}" disabled class="form-control form-control-color form-control-sm d-inline-block" style="width: 30px; height: 20px; vertical-align: middle;">`,
      },
      { label: "Display size", value: `${defaultVariation.display_size_mm}mm` },
      { label: "Display type", value: defaultVariation.display_type },
      {
        label: "Resolution",
        value: `${defaultVariation.resolution_h_px}px x ${defaultVariation.resolution_w_px}px`,
      },
      {
        label: "Memory (RAM x ROM)",
        value: `${bytesToMB(defaultVariation.ram_bytes)}MB x ${bytesToMB(
          defaultVariation.rom_bytes
        )}MB`,
      },
      ...
      { label: "Battery", value: `${defaultVariation.battery_life_mah}mAh` },
      {
        ...
        value: `${defaultVariation.water_resistance_value} ${defaultVariation.water_resistance_unit}`,
      },
      ...
      {
        label: "Band color",
        value: `<input type="color" value="${defaultVariation.band_color}" disabled class="form-control form-control-color form-control-sm d-inline-block" style="width: 30px; height: 20px; vertical-align: middle;">`,
      },
      {
        label: "Weight",
        value: `${mgToGrams(defaultVariation.weight_milligrams)}g`,
      },
      {
        label: "Release date",
        value: new Date(defaultVariation.release_date).toLocaleDateString(),
      },
    ];
    ...
    addCartBtn.click(async (e) => {
      ...
      productsContainer
        .find("#add-cart-msg")
        .text(`Error adding to cart: ${res.message}`);
    ...
    productsContainer.find(".js-buy-now-btn").click(async (e) => {
      ...
      if (cartRes.success) {
        ...
        productsContainer
          .find("#add-cart-msg")
          .text(`Error processing "Buy Now": ${cartRes.message}`);
        buyNowBtn
          .prop("disabled", false)
          .html(`<i class="uil uil-money-bill"></i> Buy now`);
      }
    });
  } catch (error) {
    console.error("Error rendering product:", error);
    productsContainer.html(
      `<div class="col-12"><p class="alert alert-danger text-center">Error loading product details. Please try again later.</p></div>`
    );
    productSpecsContainer.html(
      `<li class="list-group-item text-center">Error loading specifications.</li>`
    );
  }
}
